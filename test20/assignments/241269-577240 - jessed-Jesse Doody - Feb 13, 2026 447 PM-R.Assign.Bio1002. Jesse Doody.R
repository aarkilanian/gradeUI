...
sap_reg_browse <- sap_clean %>%
  ...
  print()
#12b) Highest: Northern peninsula forest Lowest: Strait of Belle Isle Barrens
...
sap_reg_height <- sap_clean %>%
  ...
  print()
#q17. Black spruce has higher highs and lower lows whereas Balsam fir is 
# more all around. 
sap_reg_height_low <- sap_reg_height %>%
  filter(AverageHeight < 20) %>%
  print()
#Q18. Northern peninsula forest and Western Forest < 20cm

sap_spe_browse <- sap_clean %>%
  group_by(Species) %>%
  summarize(AverageBrowsing = mean(BrowsingScore)) %>%
  print()
#Q14. Highest Black ash Lowest Black Spruce
