...
sap_spe_tally <- sap_clean %>%
  ...
  tally() %>%
  print()
# (A) The sapling data set is not evenly distributed, some ecoregions are over represented like northshore forests, 
#while others are under represnted like maritime barrens.

# (B) Recognizing bias is important because uneven sampling can lead to misleading
#conclusions about browsing pressure and species preference.
#part3
library(dplyr)
...
# (question 23)The figure shows evidence supporting the hypothesis, at low moose densities,
# browsing is more selective, while at higher densities browsing becomes more uniform.

# Moose favor Alder the most and black spruce the least, black ash is not shown on the
#graph because it lacks sufficient data. 
...
# The trend I see is that higher moose density corresponds to a larger amount of collisions
# there is one outlier, where a lower moose density has the highest amount of collisions.
...
# likely because roads overlap more with moose habitat.

# This trend makes sense in Newfoundland, where rural areas have fewer people
# but relatively high moose densities and frequent road overlap with forests.