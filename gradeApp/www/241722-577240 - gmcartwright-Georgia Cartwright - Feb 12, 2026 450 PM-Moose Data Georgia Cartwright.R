...
sap_reg_browse <- sap_clean%>%
  ...
  print()
#identify regions with highest and lowest browsing scores 
avg_browse_reg <- arrange(sap_reg_browse, desc(BrowsingScore))
#lowest browsing occurred in StraightOfBelleIsleBarrens
#highest browsing occurred in Northern_Peninsula_Forests
...
#Both species show similar patterns across ecoregions, with high and low browsing occurring in the same regions. The main difference is that Black spruce is primarily found the Western Forests 
...
sap_spe_tally <- sap_clean%>%
  ...
  tally()%>%
  print()
# The sapling study foes not appear evenly distributed. Balsam_Fir is over represented compared to Black_Ash, and North Shore Forests has more samples than Strait of Belle Isle Barrens
# Recognizing bias is important because uneven sampling can affect the reliability of conclusions about browsing scores across ecoregions
# part 3 
...
sum_spe_browse <- moose_sap%>%
  ...
  print()
# question 23
# a) There is evidence supporting the hypothesis, as areas with higher moose density show higher average browsing across species.
# b) Moose appear to prefer Willow the most due to its consistently high browsing score, while Black Spruce is the least preferred.
# c) Black Ash is not shown because it was only sampled once in the study.
# analyze moose vehicle collisions
collisions2020 <- c(56, 60, 14, 36, 48, 10, 40, 110, 6)
human_pop <- c(18000, 12000, 4000, 75100, 24000,3500, 32000, 270000,    2300)
study_sites <- c("North_Shore_Forests","Northern_Peninsula_Forests", "Long_Range_Barrens","Central_Forests","Western_Forests","EasternHyperOceanicBarrens","Maritime_Barrens","Avalon_Forests","StraitOfBelleIsleBarrens")
moose_coll <- data.frame(collisions2020, human_pop, study_sites)
# rename the column to match Ecoregion
...
#create a scatter plot of moose density vs collisions
...
# As moose density increases, collisions generally increase. One noticeable outlier occurs around density 1 with 110 collisions.
...
# The pattern I see is that collisions tend to be higher in areas with lower human population, likely because moose are more common in less populated regions.